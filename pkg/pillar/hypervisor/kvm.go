...
const (
	...
	// vc4RenderNode is the default DRM render node for Raspberry Pi vc4 GPU
	vc4RenderNode = "/dev/dri/renderD128"

	// gopRomDir is where a proprietary Intel GOP option-ROM can be placed
	// for pre-OS UEFI framebuffer output.  Lives under /persist/vault so
	// it is encrypted-at-rest; that means the ROM is only available
	// AFTER vault unlock — i.e. domains that depend on it cannot start
	// until the vault is up.  The filename is specified via the
	// IGPUGOPFile global config item; the validator rejects path
	// separators to prevent directory traversal.
	gopRomDir = "/persist/vault/gop"

	// igdRomPath is the bundled OSS IGD option-ROM built from VfioIgdPkg.
	// It provides only IgdAssignmentDxe, which sets ASLS / BDSM / GTT
	...
	// unquota'd on-disk file; 1 GiB is ample for a qemu core without guest RAM
	// (the default) and small enough to bound a crash loop before pillar
	// compresses and rotates it.
	qemuProcessCoreRlimitBytes = 1 << 30

	// qemuTraceDir holds per-VM simpletrace logs, in the encrypted vault
	// alongside crash dumps (they can leak addresses / secrets). Available only
	// after vault unlock, which is fine since domains run post-unlock.
	qemuTraceDir = "/persist/vault/qemu-trace/"
)
...
func bootOrderToFwCfgString(bo zcommon.BootOrder) string {
	switch bo {
	case zcommon.BootOrder_BOOT_ORDER_USB:
		return "usb"
	case zcommon.BootOrder_BOOT_ORDER_NOUSB:
		return "nousb"
	default:
		return ""
	}
...
[machine]
  type = "{{.Machine}}"
  dump-guest-core = "{{if .DumpGuestCore}}on{{else}}off{{end}}"
{{- if eq .Machine "virt" }}
  accel = "kvm:tcg"
  gic-version = "host"
{{- end -}}
...
{{- /* Spoof the q35 ICH9-LPC bridge's PCI device ID so a proprietary
       Intel GOP option-ROM accepts our virtual platform.  IntelGopDriver
       in OEM ROM bundles checks the LPC bridge device ID against a
       per-platform whitelist before binding to the iGPU; QEMU's default
       0x2918 is not on any real Intel platform whitelist.  Set only on
       the proprietary GOP path — applying this on the OSS path actively
       breaks the Windows display driver (different PCH ID changes its
       quirk-table path).  Risks ACPI / power-management mismatches in
       the guest if the spoofed PCH does not match the guest OS's
       expectations. */}}
[global]
  driver = "ICH9-LPC"
  property = "x-device-id"
  value = "{{.IgpuLpcDeviceID}}"
{{- end }}
...
{{- if and .BootOrder (eq .VirtualizationMode "FML") (ne .Machine "virt") }}
# Enable boot order configuration via fw_cfg.
# This setting is only applicable when using OVMF firmware (FML virtualization mode).
# The "virt" machine type (used on ARM) uses an unpatched OVMF without EveBootOrderLib,
# so fw_cfg boot order is not supported on ARM.
# OVMF's EveBootOrderLib will read this to adjust boot device priority.
# Supported values: "usb" (prioritize USB) or "nousb" (deprioritize USB)
[fw_cfg]
...
[chardev "efi-debug"]
  backend = "file"
  path = "` + kvmStateDir + `{{.DomainConfig.DisplayName}}/efi-debug.log"
...
{{end}}
{{- if .DomainConfig.EnableVnc }}
{{- /* Use virtio-vga for better VNC resolution - requires virtio_gpu driver in guest */}}
{{- /* virtio-vga provides VGA compatibility with virtio-gpu acceleration */}}
{{- if ne .Machine "virt" }}
[device "video0"]
  driver = "virtio-vga"
  bus = "pcie.0"
...
{{- if .DomainConfig.RenderNode }}
  rendernode = "{{.DomainConfig.RenderNode}}"
{{- end }}
{{- end }}
{{- end }}
{{- if .HasIntelIGPU }}
{{- /* Intel iGPU passthrough requires guest BDF 00:02.0 for VBIOS / GOP
       ROM initialisation; move the USB root port out of slot 0x2 to free
       it.  0x1b is unused on q35 and far enough from low slots that it
       does not collide with any pre-existing assignment. */}}
[device "pci.2"]
...
type tQemuGlobalConfContext struct {
	...
	EFIDebug               bool   // true when debug.enable.efi is set; wires isa-debugcon at 0x402 to capture OVMF DEBUG() output
	DumpGuestCore          bool   // true when debug.qemu.dump.guest.core is set; includes guest RAM in a qemu process core (mode B)
	HasIntelIGPU           bool   // true when Intel iGPU is in the passthrough list; moves USB root port from slot 0x2 to 0x1b
	IgpuLpcDeviceID        string // optional q35 ICH9-LPC device ID spoof for proprietary IntelGopDriver PCH whitelist (e.g. "0xa082"); empty = QEMU default 0x2918
	types.DomainConfig
	types.DomainStatus
}
...
type tQemuPCIPassthruContext struct {
	...
	Xvga         bool
	Xopregion    bool
	Romfile      string // path to GOP option-ROM (proprietary or bundled), empty = no romfile= line
	Bus          string
	Addr         string
...
func (ctx KvmContext) Setup(status types.DomainStatus, config types.DomainConfig,
	...
	// core_pattern (installed by domainmgr) points at the encrypted vault; a
	// non-pipe core_pattern only dumps a process whose RLIMIT_CORE is non-zero,
	// so raising it here targets qemu alone.  The cap bounds the raw file that
	// the kernel writes with no quota of its own; pillar compresses and rotates
	// it afterwards.  Guest RAM is excluded from this core unless
	// debug.qemu.process.core.guest.ram is set (the dump-guest-core machine
	// prop), which keeps the typical process core small.  Note the cap is fixed
	// even when that debug knob is on: for a VM with more than ~1 GiB RAM the
	// process core is truncated at the rlimit, so use the (full, streamed) guest
	// core for guest RAM; the guest-RAM process core is a best-effort debug aid.
	if globalConfig != nil && globalConfig.GlobalValueBool(types.QemuProcessCore) {
		p := spec.Get().Process
		p.Rlimits = append(p.Rlimits, specs.POSIXRlimit{
			Type: "RLIMIT_CORE",
			Hard: qemuProcessCoreRlimitBytes,
			Soft: qemuProcessCoreRlimitBytes,
		})
	}
...
// gdbSocketPath is the per-domain gdbstub UNIX socket (debug.qemu.gdb, and the
// pause-on-crash hold). Under kvmStateDir so it is cleaned with the domain.
func gdbSocketPath(domainName string) string {
	return kvmStateDir + domainName + "/gdb"
}

// qemuTracePresets maps @<name> shorthand tokens to canonical qemu trace-event
// globs, so operators can request a curated set without memorizing event names.
// A debug.qemu.trace.events value may mix @presets and bare event names.
var qemuTracePresets = map[string][]string{
	// VFIO + intel-iommu DMA-mapping flux: IOTLB invalidations, the
	// unmap-all-then-rebuild replay path (the unmap/rebuild race), per-DMA-region add/del.
	"iommu": {
		"vtd_inv_desc",
		"vtd_iommu_replay",
		"vtd_iotlb_*",
	...
	// PCI BAR-mapping flux + PM transitions: the vfio-pci <-> KVM EPT race
	// behind KVM_RUN -EFAULT on the first MMIO after a PM gap, plus the
	// vfio-mmap-fault angle (the qemu-SIGBUS angle).
	"barmap": {
		"vfio_listener_region_*",
		...
		"vfio_quirk_*",
		"memory_region_*",
		"pci_update_mappings_*",
	},
	// Device lifecycle distinct from DMA/BAR: interrupts (INTx/MSI/MSI-X),
	// reset (FLR/PM/hot-reset), and display/EDID — for device-init / reset /
	// IRQ / scanout problems (Code 43, missing IRQs, scanout corruption).
	"vfio": {
		"vfio_intx_*",
		"vfio_msi_*",
		"vfio_msix_*",
...
func setupQemuTrace(domainName, traceEvents string) ([]string, error) {
	if err := os.MkdirAll(qemuTraceDir, 0700); err != nil {
		return nil, fmt.Errorf("mkdir %s: %w", qemuTraceDir, err)
	}
	eventsFile := kvmStateDir + domainName + "/qemu-trace-events"
	var events []string
	for _, tok := range strings.Split(traceEvents, ",") {
		tok = strings.TrimSpace(tok)
		if tok == "" {
			continue
		}
		if strings.HasPrefix(tok, "@") {
			preset, ok := qemuTracePresets[tok[1:]]
			if !ok {
				return nil, fmt.Errorf("unknown trace preset %q (known: %s)",
					tok, strings.Join(qemuTracePresetNames(), ", "))
			}
			events = append(events, preset...)
		} else {
			events = append(events, tok)
		}
	}
	if len(events) == 0 {
		return nil, fmt.Errorf("no trace events after expansion")
	}
	if err := os.WriteFile(eventsFile, []byte(strings.Join(events, "\n")+"\n"), 0644); err != nil {
		return nil, fmt.Errorf("write %s: %w", eventsFile, err)
	}
	traceFile := fmt.Sprintf("%s%s.%s.trace", qemuTraceDir, domainName, time.Now().UTC().Format("20060102-150405"))
	return []string{"-trace", "events=" + eventsFile + ",file=" + traceFile}, nil
}

func qemuTracePresetNames() []string {
	names := make([]string, 0, len(qemuTracePresets))
	for k := range qemuTracePresets {
...
func (f *pciAssignmentsTemplateFiller) do(pciAssignments []pciDevice) error {
	...
	for _, pa := range pciAssignments {
		...
		if vendor, err := pa.vid(); err == nil && vendor == "0x8086" && pciPTContext.Xvga {
			...
			switch {
			case missing:
				logrus.Warnf("iGPU passthrough: configured GOP ROM %q "+
					"missing under %s for domain %s — falling back to "+
					"bundled OSS ROM", f.gopRomFilename, gopRomDir, f.domainUUID)
			case path == igdRomPath:
				logrus.Infof("iGPU passthrough: domain %s using bundled "+
					"VfioIgdPkg ROM (OS display works; no pre-OS UEFI "+
					"framebuffer)", f.domainUUID)
			default:
				logrus.Infof("iGPU passthrough: domain %s using "+
					"proprietary GOP ROM %s", f.domainUUID, path)
			}
		}

		if isIntelIGPU {
			// Intel iGPU must appear at guest BDF 00:02.0 for VBIOS / GOP
			// ROM init.  Place directly on pcie.0 — no pcie-root-port
			// wrapper.  The USB root port was relocated from 0x2 to 0x1b
			// in qemuGlobalConfTemplate above.
			pciPTContext.Bus = "pcie.0"
			pciPTContext.Addr = "0x2"
			// Debug knob: force all iGPU BAR access to trap (no mmap) so the
			// guest's MMIO writes become visible in the qemu trace.
			pciPTContext.NoMmap = f.igpuNoMmap
			if f.igpuNoMmap {
...
// gopRomPath resolves the iGPU option-ROM path:
//
//   - filename empty                          → bundled OSS ROM, no warning
//   - filename non-empty, file exists         → /persist/vault/gop/<filename>
//   - filename non-empty, file does NOT exist → bundled OSS ROM,
//     missing=true so the caller can warn the operator
//
// The second return value distinguishes "operator did not configure a
// proprietary ROM" (missing=false) from "operator configured a ROM
// that is not on disk" (missing=true) — the latter is almost certainly
// a config typo or a vault that has not unlocked yet.
func gopRomPath(filename string) (path string, configuredButMissing bool) {
	if filename == "" {
		return igdRomPath, false
	}
	candidate := filepath.Join(gopRomDir, filename)
	if _, err := os.Stat(candidate); err != nil {
		return igdRomPath, true
	}
	return candidate, false
}

// igpuIDOverride is the on-disk JSON schema for /persist/vault/gop/id.json.
//
// LpcDeviceID — emitted as `-global ICH9-LPC.x-device-id=<value>` on the
// QEMU command line, which spoofs the q35 LPC bridge's PCI device ID.
// Needed because the proprietary IntelGopDriver in OEM ROMs reads the
// LPC bridge device ID at PCI 00:1f.0 to decide whether the platform is
// on its whitelist.  QEMU's default ICH9-LPC ID 0x2918 is not on any
// real-platform whitelist, so without this override IntelGopDriver
// silently refuses to bind and there is no UEFI framebuffer.
//
// Accepts hex (with or without 0x prefix) or decimal numbers parseable
// by strconv.ParseUint(base=0).  Example file:
//
...
func (ctx KvmContext) CreateDomConfig(domainName string,
	...
	// Read boot order from VM config.
	// Boot order precedence is resolved in parseconfig.go:
	//   1. LPS (Local Profile Server) - per-app override
	//   2. Controller API (VmConfig.BootOrder) - per-app setting
	//   3. Device configuration property (app.boot.order) - device-wide default
	...
	// path — i.e. igpu.gop names a file that exists.  On the OSS path
	// (no proprietary IntelGopDriver in the option-ROM) there is no PCH
		...
		if _, missing := gopRomPath(gopRomFilename); !missing {
			...
			if igpuLpcID != "" {
				logrus.Warnf("iGPU passthrough: applying LPC device ID "+
					...
					"ACPI / power-management mismatches in the guest)",
					igpuLpcID, domainName)
			}
		}
	}
...
func (ctx KvmContext) Cleanup(domainName string) error {
	...
	// Drop the crash channel so a restarted VM with the same name gets a fresh
	// one (the qmpEventHandler goroutine for the old qemu has already exited on
	// socket close).
	crashRegistry.forget(domainName)
	return nil
}

// WatchCrash returns the channel on which the domain's QMP monitor reports a
// mode-A crash (STOP -> internal-error). domainmgr consumes it to run
// capture-first-then-policy.
func (ctx KvmContext) WatchCrash(domainName string) <-chan types.DomainCrashEvent {
	return crashRegistry.watch(domainName)
}

// DumpGuestMemory streams the guest's physical RAM as an ELF core to w.
func (ctx KvmContext) DumpGuestMemory(domainName string, w io.Writer) error {
	return execDumpGuestMemoryStream(GetQmpExecutorSocket(domainName), w)
}

// GetDomainRunState returns the raw QMP run-state, the pollable backstop for a
// missed WatchCrash event.
func (ctx KvmContext) GetDomainRunState(domainName string) (string, error) {
	return readQemuRunState(GetQmpExecutorSocket(domainName))
}
